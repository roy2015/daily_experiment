 ...
 */
public class TestSolution1262 {
    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(TestSolution1262.class);
    ...
    static class Solution {
         ...
         * 总和算减法，求余减余法，有点烧脑~
         *
         * 执行用时：14 ms, 在所有 Java 提交中击败了39.39%的用户
         * 内存消耗：47.4 MB, 在所有 Java 提交中击败了32.93%的用户
         * 通过测试用例：40 / 40
         *
         * @param nums
         * @return
        ...
        public int maxSumDivThree(int[] nums) {
            ...
            if (sumMod == 0) {
                ...
                //减去 1个1 or 2个2
                int choice1, choice2;
                choice1 = choice2 = Integer.MAX_VALUE;
                if (mod1List.size() > 0) {
                    choice1 = mod1List.get(0);
                }
                if (mod2List.size() >1) {
                    choice2 = mod2List.get(0) + mod2List.get(1);
                }
                int choice = Math.min(choice1, choice2);
                return  sum - choice;
            } else {
                //减去 2个1 or 1个2
                int choice1, choice2;
                choice1 = choice2 = Integer.MAX_VALUE;
                if (mod1List.size() > 1) {
                    choice1 = mod1List.get(0) + mod1List.get(1);
                }
                if (mod2List.size() > 0) {
                    choice2 = mod2List.get(0);
                }
                int choice = Math.min(choice1, choice2);
                return sum - choice;
            }
